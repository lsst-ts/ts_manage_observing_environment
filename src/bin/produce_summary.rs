...
use ts_observing_environment::{
    ...
    sasquatch::log_summary::{AvroSchema, Summary},
};
...
async fn main() {
    ...
    log::info!("Producing summary...");

    let sr_settings = SrSettings::new(get_schema_registry_url());
    let subject_name = "lsst.obsenv.summary-value";
    let topic_name = "lsst.obsenv.summary";
    let summary_data = Summary::default().with_ts_config_ocs("develop");
    let supplied_schema = SuppliedSchema {
        name: Some(subject_name.to_string()),
        schema_type: SchemaType::Avro,
        schema: summary_data.get_avro_schema(),
        references: vec![],
        properties: None,
        tags: None,
    };
    ...
    match avro_encoder
        .encode_struct(summary_data, &key_strategy)
        .await
    {
        Ok(bytes) => {
